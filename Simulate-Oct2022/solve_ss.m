...
%% SOLVE_SS solve all steady state, exactly  !!!!There is sth wrong with L1, S.L1 ...
    dif = 1e-2;
    steps = pa.L/dif -1;
    ...
    for i = 1: steps
        ...
        solution = fsolve( @(un)ss(un, [L1(i); pa.L-L1(i)] , pa) , [ [1;1] [0.7; 0.7] [0.9; 0.8]  ]);
    ...
    % bisection
     ...
     solution = fsolve( @(un)ss(un, [b; pa.L-b ], pa) , [ [1;1] [0.7; 0.7] [0.9; 0.8]  ]);
     ...
     if wagedif ~= 0 % we can apply bisection to find better solution
         ...
         while a-b > tol && iter < 1e3
            iter = iter + 1;
            mid = (a+b)/2;
            solution = fsolve(@(un)ss(un, [mid; pa.L-mid], pa) , [ [1;1] [0.7; 0.7] [0.9; 0.8]  ]);
            wagedif =  solution(1,1) - solution(2, 1);
